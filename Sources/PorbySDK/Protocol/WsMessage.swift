...
public enum WsMessage: Sendable {
    ...
    case log(PorbyLogEntry)
    case logBatch([PorbyLogEntry])
    case logReplay([PorbyLogEntry])
    case ping
    case pong
...
extension WsMessage: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        case .log:
            self = .log(try container.decode(PorbyLogEntry.self, forKey: .payload))
        case .logBatch:
            self = .logBatch(try container.decode([PorbyLogEntry].self, forKey: .payload))
        case .logReplay:
            self = .logReplay(try container.decode([PorbyLogEntry].self, forKey: .payload))
        case .ping:
            self = .ping
        case .pong:
            self = .pong
...
public enum ViewerCommand: Sendable {
    case setLogLevel(PorbyLogLevel)
    case setCategoryLevel(category: PorbyCategory, level: PorbyLogLevel)
    case setEnabled(enabled: Bool, category: PorbyCategory)
    case clearLogs
...
extension ViewerCommand: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        switch type {
        case .setLogLevel:
            self = .setLogLevel(try container.decode(PorbyLogLevel.self, forKey: .level))
        case .setCategoryLevel:
            let category = try container.decode(PorbyCategory.self, forKey: .category)
            let level = try container.decode(PorbyLogLevel.self, forKey: .level)
