...
final class NetworkCollector: LogCollector {
    ...
    func start() {
        ProbyURLProtocol.onLogCallback = { [weak self] entry in
            self?.onLog?(entry)
        }
        URLProtocol.registerClass(ProbyURLProtocol.self)
    }

    func stop() {
        URLProtocol.unregisterClass(ProbyURLProtocol.self)
        ProbyURLProtocol.onLogCallback = nil
    }
}

// MARK: - ProbyURLProtocol

/// Custom URLProtocol that intercepts URL requests for logging purposes.
/// Uses a "ProbyHandled" flag to prevent infinite recursion.
final class ProbyURLProtocol: URLProtocol {
    ...
    static var onLogCallback: ((ProbyLogEntry) -> Void)?

    // MARK: - Constants

    private static let handledKey = "ProbyHandled"
...
extension ProbyURLProtocol: URLSessionDataDelegate {
